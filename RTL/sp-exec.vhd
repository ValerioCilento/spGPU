...
architecture RTL of spEXEC is
    ...
    signal acc_finish_vec : std_logic_vector(N_Accelerators-1 downto 0); 
    ...
    -- ==========================================
    -- SEGNALI INTERNI PER IL CLIPPING
    -- ==========================================
    signal pixel_x_int     : std_logic_vector(N_pixel-1 downto 0);
    signal pixel_y_int     : std_logic_vector(N_pixel-1 downto 0);
    signal pixel_color_int : std_logic_vector(N_color-1 downto 0);
    signal pixel_valid_int : std_logic;

    component LINE_ACC is 
        generic(
            N_pixel : integer
        );
        Port ( 
...
begin
    ...
    -- Assegno l'OR dei valid al segnale interno
    pixel_valid_int <= pixel_valid_wire(0) or pixel_valid_wire(1) or pixel_valid_wire(2) or pixel_valid_wire(3) or pixel_valid_wire(4) or pixel_valid_wire(5);
    ...
    -- ==========================================
    -- MULTIPLEXER VERSO SEGNALI INTERNI
    -- ==========================================
    pixel_out_proc : process(acc_busy_vec, pixel_wire_x, pixel_wire_y, pixel_color_wire)
    begin
        case acc_busy_vec is
            when "000001" => --Pixel
                pixel_x_int <= pixel_wire_x(0);
                pixel_y_int <= pixel_wire_y(0);
                pixel_color_int <= pixel_color_wire(0);
            when "000010" => --Line
                pixel_x_int <= pixel_wire_x(1);
                pixel_y_int <= pixel_wire_y(1);
                pixel_color_int <= pixel_color_wire(1);
            when "001000" => --Filled Triangle
                pixel_x_int <= pixel_wire_x(3);
                pixel_y_int <= pixel_wire_y(3);
                pixel_color_int <= pixel_color_wire(3);
            when "010000" => --Circle
                pixel_x_int <= pixel_wire_x(4);
                pixel_y_int <= pixel_wire_y(4);
                pixel_color_int <= pixel_color_wire(4);
            when "100000" => --Filled Circle
                pixel_x_int <= pixel_wire_x(5);
                pixel_y_int <= pixel_wire_y(5);
                pixel_color_int <= pixel_color_wire(5);
            when others   => 
                pixel_x_int <= (others => '0');
                pixel_y_int <= (others => '0');
                pixel_color_int <= (others => '0');
        end case;
    ...
    begin
        -- Cast a integer delle coordinate
        x_coord := to_integer(unsigned(pixel_x_int));
        y_coord := to_integer(unsigned(pixel_y_int));
        
        -- Calcolo le righe minime e massime assegnate a questa tile
        -- (Moltiplicando l'indice per 24: es. Tile 0 -> 0-23, Tile 1 -> 24-47, ecc.)
        y_min := to_integer(unsigned(tile_index)) * 24;
        y_max := y_min + 23;
        
        -- I segnali vengono passati sempre per non lasciare latches (tranne il valid)
        ...
        if pixel_valid_int = '1' then
            ...
            if (x_coord >= 0 and x_coord < 320) and (y_coord >= y_min and y_coord <= y_max) then
                ...
                pixel_valid_o <= '0'; -- Forza a zero se fuori dalla tile
            end if;
        else
            pixel_valid_o <= '0';
        end if;
